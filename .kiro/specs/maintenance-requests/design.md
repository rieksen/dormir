...
```
┌─────────────────┐         ┌──────────────────┐         ┌─────────────────┐
│   React/Vite    │◄───────►│  FastAPI Backend │◄───────►│ PostgreSQL (via │
│   Frontend      │   HTTP  │   + SQLModel     │  SQL    │   SQLModel)     │
│   (TypeScript)  │  /JSON  │                  │         │  psycopg2       │
└─────────────────┘         └──────────────────┘         └─────────────────┘
        │                            │
        │                            │
        ▼                            ▼
...
- PostgreSQL behavior: Will enforce FK constraints if defined
...
0. Install psycopg2-binary: `pip install psycopg2-binary`
0. Create PostgreSQL database: `createdb maintenance_requests_db`
0. Update `database.py` DATABASE_URL to PostgreSQL connection string

**Phase 1: Backend Foundation**
1. Create `models_maintenance.py` with all model classes
2. Update `database.py` to import MaintenanceRequest model and configure PostgreSQL
...
- PostgreSQL setup: Ensure PostgreSQL server is running and database exists before application starts
- Connection string: Update DATABASE_URL in `database.py` to point to PostgreSQL instance
- Migration strategy: Add new table without disrupting existing data


**Environment Configuration:**
- Backend: Set DATABASE_URL environment variable for PostgreSQL connection string (e.g., `postgresql://user:password@localhost/maintenance_db`)
- Backend: Ensure psycopg2-binary is installed as dependency for PostgreSQL driver
